...
*/	
	/* function updateBtn met 3 parameters: het wijzigen van de staat van de knop van 'lnk' naar 'hvr' en andersom.
	
	fromBtn is de staat van de knop
	toBtn is de staat waar hij 'heen' moet (hvr of lnk).
	aTag is de plek waarop het event plaatsvindt.
	
	imageObject = als het event plaatsvindt, haal de eerste "img" op.
	oldBtn = de src van de opgehaalde "img", dus de 'huidige' knop en de huidige staat.
	newBtn = de "img" die is opgehaald door imageObject, zoek de src...en vervang deze van de huidige staat van de knop naar de andere staat van de knop.
...
/*	
	...
	for(hetEvent of eventLijst){ // Voor elke var hetEvent in de array 'eventLijst'...
		...
		for(var i = 0; i<btnParts.length; i++){ // Ga door alle a-tags heen in de nav...
			...
			}, false);  // btnParts[i].addEventListener(hetEvent, function(e)
